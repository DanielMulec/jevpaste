...
|---|---|---|
| deciding | Jev chooses `nothing_fits`, at any Narrowing step ([narrowing.md](narrowing.md)) | stop clocks; `showNoSuitableMatchOffer`; schedule 8 s → offeringDirectPaste |
| offeringDirectPaste | `onAccept` (Enter, focus back in the Target's app) | `deliver(OuterLineBreaks.stripped(from: item))` → delivering (Bound Target re-verified there) |
...
Text: `OuterLineBreaks.stripped(from:)` (pure stripping, shared with every whole-copy paste of Narrowing).
...
The offer only follows Narrowing: `SmartPastePath.noSuitableMatchOfferEnd: NoSuitableMatchOfferEnd?`
(`accepted, dismissed, timedOut`; ⌘⇧V = dismissed). `endingOffer(_:)` sets it into the attempt's stored path,
keeping the Narrowing trace. `logFragment` renders `via=directPaste reason=enterAfterNoMatch p=0.12` (accepted, any
outcome) and `via=narrowing steps=1 calls=1 p=0.12 questions=1 offer=dismissed|timedOut`; `p=` is omitted when nil. Not a `PasteAttemptNote`: notes are visible text.
...
## Tests
Core (`NoSuitableMatchOfferTests`, fakes): `nothing_fits` at step 1 and at a later step shows the offer with the Bound Target, no outcome,
...
dismisses, no Jev request; Target changed → `.failed(.targetChanged)`; note still carried. `OuterLineBreaks`:
`OuterLineBreaksTests`. Four existing tests add one `presenter.dismissOffer()` line.
