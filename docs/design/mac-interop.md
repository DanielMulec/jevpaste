...
- Chrome: `data:text/html,<textarea autofocus></textarea>`; Daniel clicks in, presses ⌘⇧V. Ghostty: a scratch Herdr
  ...
  each: I place a synthetic 3-type original (string, html, custom type) — Daniel's clipboard is
...
4. ~~Surrounding text only when no label contract exists~~ → always (GATE A). 5. Placing a synthetic original on the clipboard.
