...
final class FocusedTargetResolver<Source: FocusSource> {
    ...
    /// When each process was asked to wake; entries older than the wake window are pruned on every read.
    private var wakeRequestedAt: [Int32: ContinuousClock.Instant] = [:]
    ...
    private func resolutionWhileFocusIsUnreadable() -> TargetResolution {
        ...
        wakeRequestedAt[application.processIdentifier] = now()
        return waking
    }

    private func isWithinWakeWindow(of processIdentifier: Int32) -> Bool {
        let currentTime = now()
        wakeRequestedAt = wakeRequestedAt.filter { currentTime - $0.value < Self.wakeWindow }
        return wakeRequestedAt[processIdentifier] != nil
    }
}
