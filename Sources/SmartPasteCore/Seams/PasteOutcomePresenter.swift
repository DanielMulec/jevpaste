/// How the shell learns what to show for a Paste Attempt: the waking and processing indicators, the retrying state,
/// the start of delivery, the visible outcome and the No Suitable Match offer. All of it leaves key focus with the
/// Target, except the offer, which takes key focus (without activating the app) so Enter and Esc reach it.
///
...
public protocol PasteOutcomePresenter {
    ...
    /// Shows the waking indicator during the Wake Wait, naming the app whose focus is not readable yet. Cancelled
    /// like the processing indicator: `onCancel` is called on a click; Esc never reaches it.
    ...
    /// diagnostics and never changes what is shown. `wakeWait` is how long the attempt waited for a readable focus,
    /// `nil` when it was readable at ⌘⇧V; diagnostics only as well.
    func showOutcome(
