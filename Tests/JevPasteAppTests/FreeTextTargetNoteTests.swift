...
@MainActor
struct FreeTextTargetNoteTests {
    @Test(arguments: [
    ...
    func theLogLineNamesTheNote(note: PasteAttemptNote) {
        let line = IndicatorPresenter.outcomeLogLine(.inserted, note: note, path: .jev(freeTextProbability: 0.1))
        #expect(line == "outcome inserted via=jev p=0.10 note=\(note)")
    }
}
