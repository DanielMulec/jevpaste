...
/// The Candidates `LineAndLabelCandidateExtraction` derives from `text`, as plain strings.
func derivedCandidateTexts(of text: String) -> [String] {
    LineAndLabelCandidateExtraction().candidates(in: ClipboardItem(text: text)).map(\.text)
}

struct CandidateDerivationTests {
    @Test func eachNonBlankLineIsACandidateInDocumentOrder() {
        #expect(derivedCandidateTexts(of: "Ada Lovelace\nAnalyst\nLondon") == ["Ada Lovelace", "Analyst", "London"])
    }

    @Test func leadingAndTrailingSpacesAndTabsAreTrimmedInnerWhitespaceKept() {
        #expect(
            derivedCandidateTexts(of: "  \tAda  \t Lovelace\t \n\u{00A0}London\u{3000}") == [
                "Ada  \t Lovelace", "London",
            ])
    }
    ...
    @Test func emptySourceYieldsNoCandidates() {
        #expect(derivedCandidateTexts(of: "").isEmpty)
        #expect(derivedCandidateTexts(of: " \t\r\n ").isEmpty)
    }

    @Test func crlfAndLoneCarriageReturnSplitLinesWithoutLeavingCarriageReturns() {
        let texts = derivedCandidateTexts(of: "Ada\r\nLondon\rParis\r\n\r\nRome\u{2028}Oslo")
    ...
    @Test func identicalTextIsOfferedOnce() {
        #expect(
            derivedCandidateTexts(of: "London\nCity: London\n  London\t\nCity: London")
                == ["London", "City: London"]
        )
    }
    ...
    @Test func differentlyEncodedEqualTextIsKeptAsTwoCandidates() {
        ...
        let texts = derivedCandidateTexts(of: "\(composed)\n\(decomposed)")
    ...
    @Test func unicodeAndEmojiLinesAreKeptByteForByte() {
        ...
        let texts = derivedCandidateTexts(of: lines.joined(separator: "\r\n"))
...
struct CandidateLabelValueTests {
    @Test func labelledLineYieldsTheWholeLineAndTheValue() {
        #expect(
            derivedCandidateTexts(of: "Email: ada@example.com\nX / Twitter:\t @ada ")
    ...
    @Test func valueContainingAURLKeepsAllItsColons() {
        #expect(
            derivedCandidateTexts(of: "Website: https://a.example:8443/x?y=1:2\nTime: 10:30")
                == [
    ...
    @Test func urlLineWithoutLabelIsNotSplit() {
        #expect(derivedCandidateTexts(of: "https://x.com/ada_lovelace") == ["https://x.com/ada_lovelace"])
    }

    @Test func colonWithoutFollowingSpaceDoesNotSplit() {
        #expect(derivedCandidateTexts(of: "Ratio:3:4\nNote:done") == ["Ratio:3:4", "Note:done"])
    }
    ...
    @Test func labelLongerThanFortyCharactersDoesNotSplit() {
        ...
        #expect(derivedCandidateTexts(of: "\(fortyCharacterLabel): v") == ["\(fortyCharacterLabel): v", "v"])
        #expect(derivedCandidateTexts(of: "\(fortyOneCharacterLabel): v") == ["\(fortyOneCharacterLabel): v"])
    }

    @Test func labelContainingAColonDoesNotSplitAtALaterSeparator() {
        #expect(derivedCandidateTexts(of: "See https://a.example: details") == ["See https://a.example: details"])
    }

    @Test func labelWithEmptyValueYieldsOnlyTheLine() {
        #expect(derivedCandidateTexts(of: "Email:   \nPhone:") == ["Email:", "Phone:"])
    }
}
